...
%                         coverage(Fadel,jj)=sum(sum(Overlap))^2/X_count2(locate(1),locate(2),locate(3))/((fault(3)+1)*(fault(4)+1));
                    ...
                    RL(jj)=counter-SSnow;
                    clc;
                    fprintf('The Current ARL = %6.4f, MRL = %6.4f, after %d simulations when delta = %6.4f and SimNum = %d',mean(RL(1:jj)),median(RL(1:jj)),jj,Delta(Fadel),Fadel);
                    flag=0;
                end
            end
        end
...
save ('ARLMeanShift20SqST', 'ARL', 'MRL','Delta','fault_time','SS_Number','k',...
